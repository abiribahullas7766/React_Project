...
const BlogCard = () => {
    ...
    useEffect( () => {
        fetch("postData.json").then(res => res.json()).then(data => setBlogs(data));
    },[] )
  return (
    <div className='grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-5'>
        {
            blogs.slice(0, 3).map(blog => <div key={blog.id} className='relative cursor-pointer rounded-xl'>
                ...
                <div className='absolute bottom-0 left-0 w-full h-full bg-black/60 hover:bg-black/40 flex items-end
